package com.satan.common.web.util;
...
import com.satan.common.utils.HttpMethod;
import com.satan.common.web.exception.WebBasicCodeEnum;
import com.satan.common.web.exception.WebException;
...
public class HttpUtils {
    ...
    public static <T> T getModelFromQueryString(String queryStringJson, Class<T> type){
        T result = JSON.parseObject(queryStringJson,type);
        validate(result, type);
        return result;
    }

    public static <T> T getModelFromQueryString(HttpServletRequest request, Class<T> type){
        T result = JSON.parseObject(getQueryStringAsJson(request), type);
        validate(result, type);
        return result;
    }
...
//                }
    ...
    public static <T> void validate(T t, Class<?> type) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            throw new WebException(WebBasicCodeEnum.ILLEGAL_ARGUMENT_ERROR.getCode(),message.toString());
        }
    }
